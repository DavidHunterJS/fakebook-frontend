    pipeline {
        ...
        stages {
            stage('Initialize') {
                steps {
                    script {
                        ...
                        def branchName = params.DEPLOY_BRANCH
                        
                        // If no explicit branch provided, try to get from environment
                        if (!branchName || branchName == 'develop') {
                            branchName = env.GIT_BRANCH?.replaceAll('origin/', '') ?: 
                                        env.BRANCH_NAME ?: 
                                        sh(script: 'git branch --show-current 2>/dev/null || echo "develop"', returnStdout: true).trim()
                        }
            ...
            stage('Checkout Code') {
                steps {
                    echo "✅ Code already checked out at: ${env.WORKSPACE}"
                }
            }
            ...
            //             echo "Repository: https://github.com/DavidHunterJS/fakebook-frontend.git"
                        
            //             // Clean checkout of the specific branch
            //             checkout([
            //                 $class: 'GitSCM',
            ...
            //                     echo "✅ Repository checkout successful - package.json found"
            //                 else
            //                     echo "❌ Error: package.json not found - checkout failed"
            //                     exit 1
            //                 fi
...
=======
            ...
            stage('Create Feature Environment') {
                ...
                steps {
                    script {
                        def featureName = env.RESOLVED_BRANCH.replace('feature/', '').replaceAll('[^a-zA-Z0-9-]', '-').toLowerCase()
                        ...
                        sh """
                            ...
                            heroku config:set FEATURE_BRANCH=${env.RESOLVED_BRANCH} -a ${env.HEROKU_APP_NAME}
                        """
                    }
                }
            }
...
>>>>>>> feature/under-construction-header
                        }
                    }
                }
            }
                ...
                steps {
                    script {
                        ...
                        sh '''
                            ...
                            BRANCH_SAFE=$(echo "${RESOLVED_BRANCH}" | sed 's/[^a-zA-Z0-9-]/-/g')
                            FULL_TAG="${DEPLOY_TAG}-${BRANCH_SAFE}"
                            git tag -a "$FULL_TAG" -m "${DEPLOY_ENV} deployment ${BUILD_NUMBER} from ${RESOLVED_BRANCH} branch"
                            ...
                            heroku config:set DEPLOY_BRANCH="${RESOLVED_BRANCH}" -a ${HEROKU_APP_NAME}
        ...
        post {
            ...
            success {
                script {
                    ...
                    if (env.RESOLVED_BRANCH.startsWith('feature/') && params.CREATE_FEATURE_APP) {
                        echo """
